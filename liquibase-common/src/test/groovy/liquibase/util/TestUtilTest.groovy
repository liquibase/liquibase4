...
class TestUtilTest extends Specification {
    ...
    def "createAllPermutations"() {
        when:
        def permutations = TestUtil.createAllPermutations(DropTablesAction, [
                tables            : [[new RelationReference(Table, "table1")], [new RelationReference(Table, "table2")], [new RelationReference(Table, "table3")]],
                cascadeConstraints: [true, false],
        ])

        then:
        permutations.size() == 4 * 3 //3 + 1 null * 2 + 1 null
        assert permutations*.toString().contains("dropTables()") //empty permutation not included
        assert !permutations.contains(null) //no null value
        permutations*.toString().sort() == (permutations.collect {
            it.toString()
        }).unique().sort() //nothing is duplicated
    }

    def "createAllPermutationsWithoutNulls"() {
        when:
        def permutations = TestUtil.createAllPermutationsWithoutNulls(DropTablesAction, [
                tables            : [[new RelationReference(Table, "table1")], [new RelationReference(Table, "table2")], [new RelationReference(Table, "table3")]],
                cascadeConstraints: [true, false],
        ])
